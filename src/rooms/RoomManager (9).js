...
export class RoomManager {
  ...
  async connect(ws, opts = {}) {
    ...
    if (opts.gameId) {
      ...
      playerId = this.#assignSlot(room, ws, opts.playerName, opts.userId);
  ...
  #createRoom(isPrivate = false) {
    ...
    const room   = {
      ...
      nextAutoAt: null,
      slots: {
        player1: { ws: null, name: null, awayAt: null, reconnectTimer: null, userId: null },
        player2: { ws: null, name: null, awayAt: null, reconnectTimer: null, userId: null },
      },
    };
  ...
  #assignSlot(room, ws, name, userId = null) {
    ...
    for (const [pid, slot] of Object.entries(room.slots)) {
      if (!slot.ws && slot.name === null) {
        slot.ws     = ws;
        slot.name   = name ?? pid;
        slot.userId = userId;     // Supabase user ID — stats için
        return pid;
      }
    }
