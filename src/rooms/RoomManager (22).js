...
export class RoomManager {
  ...
  async connect(ws, opts = {}) {
    ...
    if (opts.gameId) {
      ...
      // Kuyrukta bir oda varsa o odaya katılmayı deniyoruz; oda bu arada
      // dolmuş/kapanmışsa yeni oda açıyoruz. Tüm bu işlem withRoomLock
      // altında yapılıyor, böylece iki instance aynı odayı aynı anda
      // dolduramıyor.
      const candidateGameId = await peekPublicQueue(requestedPlayerCount);
        ...
        try {
          ({ room, playerId, reconnectToken } = await withRoomLock(candidateGameId, async () => {
            ...
            if (r.playerCount !== requestedPlayerCount) {
              ...
              // ve kendimiz için yeni oda açalım.
              await dequeuePublicRoom(candidateGameId, requestedPlayerCount);
          ...
          ({ playerId, reconnectToken } = await withRoomLock(room.gameId, async () => {
            const a = this.#assignSlot(room, ws, opts.playerName, opts.userId);
            await this.#persistRoom(room);
            await enqueuePublicRoom(room.gameId, requestedPlayerCount);
            return a;
          }));
        }
      } else {
