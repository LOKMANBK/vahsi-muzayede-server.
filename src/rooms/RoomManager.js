...
export class RoomManager {
  ...
  async connect(ws, opts = {}) {
    ...
    if (opts.gameId) {
      ...
      // YA DA istenen playerCount ile uyuşmuyorsa yeni oda açıyoruz.
      // Tüm bu işlem withRoomLock altında yapılıyor, böylece iki instance
      // aynı odayı aynı anda dolduramıyor.
      //
      // NOT: Kuyruk şu an playerCount'a göre ayrı namespace'lenmiş değil
      // (redis.js'de tek bir "waiting:public" seti var). Bu yüzden bir
      // isteğin peek ettiği aday farklı playerCount'ta bir oda olabilir —
      // bu durumda adayı kuyruktan ÇIKARMIYORUZ (doğru moddaki biri onu
      // hâlâ bulabilsin diye), sadece kendimiz için yeni oda açıyoruz.
      // İleride yüksek trafikte bunu playerCount bazlı ayrı kuyruklara
      // bölmek (ör. "waiting:public:2", "waiting:public:3") performansı
      // artırır; şimdilik doğruluk için yeterli.
        ...
        try {
          ({ room, playerId, reconnectToken } = await withRoomLock(candidateGameId, async () => {
            ...
            if (r.playerCount !== requestedPlayerCount) {
              // Başka bir oyuncu sayısı için açılmış oda — bizim için değil,
              // ama kuyrukta KALSIN (doğru moddaki biri bulabilsin).
            ...
            // Oda tamamen doldu mu? Doldiyse kuyruktan çıkar.
          ...
          // Kuyruktan gelen oda uygun değil (bayat ya da yanlış mod) — yeni oda aç.
          room = this.#createRoom(false, requestedPlayerCount);
          ({ playerId, reconnectToken } = await withRoomLock(room.gameId, async () => {
            const a = this.#assignSlot(room, ws, opts.playerName, opts.userId);
            await this.#persistRoom(room);
