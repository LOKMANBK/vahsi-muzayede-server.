...
import {
  ...
  enqueuePublicRoom, peekPublicQueue, dequeuePublicRoom,
} from '../utils/redis.js';
import { logger } from '../utils/logger.js';

// monitoring.js opsiyoneldir — dosya yoksa analytics sessizce devre dışı olur.
// Top-level await kullanmıyoruz: class parse sorununu önlemek için
// capture'ı lazy bir wrapper üzerinden çağırıyoruz.
let _captureImpl = null;
async function _ensureCapture() {
  if (_captureImpl) return _captureImpl;
  try {
    const m = await import('../utils/monitoring.js');
    _captureImpl = m.capture ?? (() => {});
  } catch {
    _captureImpl = () => {};
  }
  return _captureImpl;
}
function capture(event, props, id) {
  _ensureCapture().then(fn => fn(event, props, id)).catch(() => {});
}
...
export class RoomManager {
  ...
  #botTimers    = new Map(); // gameId → bot match timer
  #subscribed   = false;
  ...
  async connect(ws, opts = {}) {
    ...
    if (opts.gameId) {
      ...
      //
      // Önce Redis'teki public waiting kuyruğuna bakıyoruz. Kuyrukta bir oda
      // varsa o odaya katılmayı deniyoruz; oda bu arada dolmuş/kapanmışsa
      // (peekPublicQueue'dan geldi ama slot dolu) yeni oda açıyoruz.
      // Tüm bu işlem withRoomLock altında yapılıyor, böylece iki instance
      // aynı odayı aynı anda dolduramıyor.
      const candidateGameId = await peekPublicQueue();

      if (candidateGameId) {
        // Kuyruktaki odayı lock altında dene
        try {
          ({ room, playerId, reconnectToken } = await withRoomLock(candidateGameId, async () => {
            const r = await this.#getOrRestoreRoom(candidateGameId);
            // Oda hâlâ uygun mu? (waiting + boş slot + public)
            if (r.status !== 'waiting' || r.private) {
              await dequeuePublicRoom(candidateGameId);
              throw new Error('STALE'); // aşağıda yakalanır, yeni oda açılır
            }
            ...
            // İkinci oyuncu geldi, oda doldu → kuyruktan çıkar
            const stillHasSlot = Object.values(r.slots).some(s => s.name === null);
            if (!stillHasSlot) await dequeuePublicRoom(candidateGameId);
            await this.#persistRoom(r);
            return { room: r, ...a };
          }));
        } catch (err) {
          if (err.message !== 'STALE') throw err;
          // Kuyruktan gelen oda artık geçerli değil — yeni oda aç
          room = this.#createRoom(false);
          ({ playerId, reconnectToken } = await withRoomLock(room.gameId, async () => {
            const a = this.#assignSlot(room, ws, opts.playerName, opts.userId);
            await this.#persistRoom(room);
            await enqueuePublicRoom(room.gameId);
            return a;
          }));
        }
      } else {
        // Kuyruk boş — yeni public oda aç ve kuyruğa ekle
        room = this.#createRoom(false);
        ({ playerId, reconnectToken } = await withRoomLock(room.gameId, async () => {
          const a = this.#assignSlot(room, ws, opts.playerName, opts.userId);
          await this.#persistRoom(room);
    ...
    if (room.status === 'waiting') {
      ...
      // Bot eşleştirme: odada yalnızca 1 oyuncu varsa timer başlat.
      // İkinci oyuncu gelirse (#handleLobbyReady içinde) timer iptal edilir.
      const slotCount = Object.values(room.slots).filter(s => s.name !== null).length;
      if (slotCount === 1 && !room.private && BOT_MATCH_DELAY_MS > 0) {
        this.#startBotTimer(room);
      } else if (slotCount === 2) {
        // İkinci gerçek oyuncu geldi — bot timer'ı iptal et
  ...
  async #handleLobbyReady(room, playerId) {
    ...
    // Gerçek oyuncu "Hazırım" dedi — bot artık gerekmez
    this.#cancelBotTimer(room.gameId);
    await this.#persistRoom(room);
    await this.#broadcastLobby(room);
      ...
      try {
        await withRoomLock(room.gameId, async () => {
          ...
          capture('game_started', {
            ...
            hasP1:    !!fresh.slots.player1.userId,
            hasP2:    !!fresh.slots.player2.userId,
          });
        });
      } catch (err) {
  ...
  //
  // BOT_MATCH_DELAY_MS=0 ise bot sistemi hiç çalışmaz.
  ...
  async #addBot(gameId) {
    ...
    await withRoomLock(gameId, async () => {
      const room = await this.#getOrRestoreRoom(gameId).catch(() => null);
      if (!room || room.status !== 'waiting') return; // oyuncu bu arada ikinci biri gelmiş olabilir
   ...
   * Standart #handleLobbyReady mantığını taklit eder.
   */
    ...
    room.lobbyCountdownTimer = setTimeout(async () => {
      ...
      try {
        await withRoomLock(room.gameId, async () => {
          ...
          capture('game_started', { gameId: fresh.gameId, isPrivate: false, hasBot: true });
        });
      } catch (err) {
  ...
  async #saveMatchStats(room) {
    ...
    capture('game_finished', {
      gameId:       room.gameId,
      winnerId,
      p1Score:      state.battle?.scores?.player1 ?? 0,
      p2Score:      state.battle?.scores?.player2 ?? 0,
      p1MmrChange,
      p2MmrChange,
  ...
  async #closeRoom(room, reason) {
    ...
    if (!room.private) await dequeuePublicRoom(room.gameId).catch(() => {});
