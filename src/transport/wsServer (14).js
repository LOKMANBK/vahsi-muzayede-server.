...
/** Yalnızca 2 veya 3 kabul edilir (bkz. RoomManager.js — 3 kişilik mod).
 *  Geçersiz/eksik değer sessizce 2'ye düşer, hata fırlatmaz. */
function validPlayerCount(val) {
  const n = Number(val);
  return n === 3 ? 3 : 2;
}
...
async function handleMessage(ws, msg, rooms) {
  switch (msg.type) {
    ...
    case 'JOIN': {
      ...
      // 3 kişilik mod: client { type:'JOIN', playerCount: 3, ... } gönderir.
      // Yalnızca YENİ oda açılırken anlamlıdır — gameId ile mevcut bir odaya
      // katılırken RoomManager odanın kendi playerCount'unu kullanır, bu
      // alanı yok sayar (bkz. RoomManager.js connect() yorumu).
      const playerCount    = validPlayerCount(msg.playerCount);
