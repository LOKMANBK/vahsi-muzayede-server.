...
async function run(){
  ...
  console.log('\n[ FREE_CHOICE — rakip parasiz ]');
  {
    // GameEngine direkt kullanarak FREE_CHOICE durumu üretiriz:
    // player2'nin bakiyesini 0 yapıp bir tur başlatırız.
    const {GameEngine}=await import('./game/GameEngine.js');
    const {GameState}=await import('./game/GameState.js').catch(()=>({}));
    const {makeInitialState,STATUS:ST}=await import('./game/GameState.js');
    const {buildQueue,beginRound}=await import('./game/AuctionEngine.js');
    const {Actions:A,applyAction:aa}=await import('./game/actions.js');

    // Manuel state kur: player2 bakiyesi=0, player1 bakiyesi=5
    const eng=new GameEngine();
    aa(eng,A.startGame({player1:'P1',player2:'P2'}));

    // İç state'e erişmek yerine engine API'sini zorlayalım:
    // player2 tüm parasını harcayacak şekilde 19 TL teklif versin ve kazansın
    // Bunun için önce tur 0'ı player2 lehine bitirmemiz lazım — mümkün değil
    // kolayca, o yüzden sadece AuctionEngine unit testini yaparız.
    const {chooseFreeItem,beginRound:br,placeBid:pb}=await import('./game/AuctionEngine.js');
    const {makeInitialState:mis}=await import('./game/GameState.js');
    const {buildQueue:bq}=await import('./game/AuctionEngine.js');

    // Sıfırdan state oluştur, player2 parasız
    let state={...mis('test_fc'),_queue:bq()};
    state={...state,players:{
      ...state.players,
      player2:{...state.players.player2,balance:0},
    }};
    const {state:s1}=br(state);
    chk('FREE_CHOICE status',s1.status==='free_choice');
    chk('FREE_CHOICE deciderId',s1.freeChoice?.deciderId==='player1');
    chk('FREE_CHOICE brokeId',s1.freeChoice?.brokeId==='player2');

    // 'take': player1 bedava alır
    const {state:s2,ok:ok2}=chooseFreeItem(s1,'player1','take');
    chk('take ok',ok2);
    chk('take winner=p1',s2.roundResult?.winnerId==='player1');
    chk('take price=0',s2.roundResult?.price===0);
    chk('take bakiye degismez',s2.players.player1.balance===20);

    // 'pass': hayvan player2'ye (parasız) gider
    const {state:s3,ok:ok3}=chooseFreeItem(s1,'player1','pass');
    chk('pass ok',ok3);
    chk('pass winner=p2',s3.roundResult?.winnerId==='player2');

    // Hatalı deciderId reddedilmeli
    const {ok:okBad}=chooseFreeItem(s1,'player2','take');
    chk('yanlis decider reddedildi',!okBad);
  }
