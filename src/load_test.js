...
// Her test öncesi Redis'i temizle (test izolasyonu)
async function flushRedis() {
  try {
    const { default: Redis } = await import('ioredis');
    const url = process.env.REDIS_URL;
    if (!url) return; // in-memory stub — temizlemeye gerek yok
...
async function main() {
  ...
  for (const [name, fn] of tests) {
    ...
    await wait(600); // testler arası temizlik süresi (IP rate limit reset için)
  }
